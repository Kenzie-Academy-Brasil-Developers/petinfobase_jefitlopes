import { createModalDelete, createModalEditPost, createModalNewPost, createModalPost, render } from "./render.js";
import { createPost, deletePost, updatePost } from "./request.js";
...
export function handleModalOld (array){
    ...
    modalOpenBtn.forEach(button =>{
        ...
        button.addEventListener('click', ()=>{
            const modalContent = createModalPost(button.dataset.postId, array);
...
export async function handleModalNewPost (){
    ...
    const modalOpenBtn = document.querySelector('.create__btn');
    let count =0;
    modalOpenBtn.addEventListener('click', async ()=>{
        modalPost.close();
            ...
            modalPost.appendChild(modalContent);
