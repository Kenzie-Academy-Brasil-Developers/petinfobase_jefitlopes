import { renderProfilePhoto, createToast, showLogout } from "./render.js";
...
export async function createUser (createBody){
    const newUser = await fetch(`${baseURL}/users/create`,{
        ...
        if(response.ok){
            createToast('Sua conta foi criada com sucesso!', 'Agora você pode acessar os conteúdos utilizando seu usuário e senha na página de login: ', "../../index.html" )
...
export async function updatePost (postBody, postId){
    const updated = await fetch(`${baseURL}/posts/${postId}`, {
        ...
        if(response.ok){
            createToast('Post editado com sucesso!','O post agora contém as novas informações passadas');
            return response.json();
        } else {
            response.json().then((responseError) =>{
            })
...
export async function deletePost (postId){
    const deleted = await fetch(`${baseURL}/posts/${postId}`, {
        ...
        if(response.ok){
            createToast('Post deletado com sucesso!','O post selecionado para exlusão foi deletado, a partir de agora não aparecerá no seu feed');
...
export async function createPost (postBody){
    const newPost = await fetch(`${baseURL}/posts/create`,{
        ...
        if(response.ok){
            createToast('Post criado com sucesso!','Seu post foi criado com sucesso, a partir de agora ele aparecerá no seu feed')
